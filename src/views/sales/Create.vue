      ...
      <v-flex xs12>
        ...
        <v-card class="elevation-12"> 
          ...
          <v-card-title primary-title>
            <h3 class="headline mb-0">Add a new bill
...
<script type="text/javascript">
  ...
  export default {
    ...
    mounted() { 
      ...
      this.form.get('/api/billings/get-latest-bill-no')
        .then(data => {
          if(data.data && data.data.bill_no)
            this.form.bill_no = parseInt(data.data.bill_no) + 1;
          else
            this.form.bill_no = 1;
        })
