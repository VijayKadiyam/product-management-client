      ...
      <v-flex xs12 sm8 md8>
        ...
        <v-card class="elevation-12"> 
          ...
          <v-card-title primary-title>
            <h3 class="headline mb-0">Edit product
...
<script type="text/javascript">
  ...
  export default {
    ...
    mounted() { 
      ...
      this.form.get(`/api/products/${this.$route.params.product_id}`)
        .then(data => {
          this.form = new Form({
            product_category_id: parseInt(data.data.product_category_id),
            qty: data.data.qty
          })
        })
    },
    ...
    methods: {
      ...
      create() {
        this.form.patch(`/api/products/${this.$route.params.product_id}`)
          .then(data  =>  {
