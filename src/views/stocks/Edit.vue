      ...
      <v-flex xs12 sm8 md8>
        ...
        <v-card class="elevation-12"> 
          ...
          <v-card-title primary-title>
            <h3 class="headline mb-0">Edit stock
...
<script type="text/javascript">
  ...
  export default {
    ...
    mounted() {
      ...
      this.form.get(`/api/stocks/${this.$route.params.stock_id}`)
        .then(data => {
          this.form = new Form({
            stock_category_id: parseInt(data.data.stock_category_id), 
            supplier_id: parseInt(data.data.supplier_id),
            qty: data.data.qty,
    ...
    methods: {
      ...
      update() {
        this.form.patch(`/api/stocks/${this.$route.params.stock_id}`)
          .then(data  =>  {
